...
if (nodeArgs.length > 2) {
    ...
    if (nodeArgs.length > 4) {
        ...
        // if only one word set the word at index 3 as searchValue
        searchValue = nodeArgs[3];
    }
} else {
    // set action to perform equal to argument value at index 2
    actionToDo = nodeArgs[2];
}
...
if (actionToDo == "my-tweets") {
    ...
    fs.readFile("random.txt", "utf-8", function(err, data) {
        // Split the file text and store first string as action and second as the value to use
        var dataReturned = data.split(",");
        //split text and store values in action todo and search value
        // This code will work if there are more than one instruction present in the file
        // loop through the array and work with two values as a time
        for (var j = 0; j < dataReturned.length;) {
            ...
            // increment counter by two to point to next pair of action and search value
            j = j + 2;
        }
